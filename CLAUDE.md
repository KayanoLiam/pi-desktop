...
- Bundle identifiers are now `io.github.kayanoliam.pi-desktop` (`.dev` / `.beta` / `.nightly` overlays) and the updater endpoints are empty, so shipped builds never poll Cline's feed. The updater `pubkey` in `tauri.conf.json` is still Cline's; generate a Pi key (`bun tauri signer generate`) before re-enabling endpoints. Do not tag releases from the inherited `desktop-publish.yml`: it is hard-wired to `cline/cline`, `main`, Slack and Azure signing. Inherited `publish-*` skills are not evidence of Pi Desktop compatibility.
