...
- **New local threads run through the user's installed `pi` CLI** (`pi --mode rpc`), since `f87141ee2`: streaming text/thinking, tool cards, opt-in tool approvals, extension dialogs, stop/queue, Pi session history (open, continue, rename, delete). Later commits added Pi slash commands through RPC (`e41d676ea`), scoped models (`bf21b316f`), `/tree` session navigation (`66d95c094`) and management of installed Pi extensions in Settings → Extensions (`7f6043b59`).
- Still on the inherited Cline runtime (`@cline/core` Hub): existing Cline sessions and SSH remote environments. Not supported for Pi threads yet: fork, editing earlier messages, editing/removing a single queued message, file checkpoints.
- Next milestones, in README order: fork/message/queue editing and SSH threads through Pi; replace or extract the remaining Cline workspace/runtime dependencies; migrate updates, signing and release automation; resolve inherited test/type/lint failures and set up desktop CI.
...
- The app is named **Pi** (`Pi Dev` / `Pi Beta` / `Pi Nightly`), the Rust crate is `pi-desktop`, bundle identifiers are `io.github.kayanoliam.pi-desktop` (`.dev` / `.beta` / `.nightly` overlays) and the updater endpoints are empty, so shipped builds never poll Cline's feed. The updater `pubkey` in `tauri.conf.json` is still Cline's; generate a Pi key (`bun tauri signer generate`) before re-enabling endpoints.
...
PI_DESKTOP_PI_BIN=/nonexistent-pi-test bun x vitest run \
  ...
  webview/hooks/chat-session/helpers.test.ts webview/lib/theme.test.ts \
...
- At `fbd38ff79`, the focused Pi command above: 369 passed / 1 failed, deterministically: `pi-model-selector.test.tsx` › "seeds a resumed session's recorded model and keeps it when the catalog lacks it".
